...
try{
 page.setDefaultTimeout(60000);await page.setViewport({width:1024,height:768});
 ...
 await page.waitForSelector('#hint-unlock',{visible:true,timeout:10000});
 await page.locator('#hint-unlock').click();
 await page.waitForFunction(()=>window.__NESI_PREFS__.value.hints[0]===1&&!window.__NESI_PLATFORM__.busy);
 assert.equal(await page.evaluate(()=>window.__NESI_DEMO_GAME__.state),'paused');
 await page.evaluate(()=>window.__reward=false);await page.locator('#hint-unlock').click();
 await page.waitForFunction(()=>!window.__NESI_PLATFORM__.busy);
 ...
 const log=await page.evaluate(()=>window.__adLog);
 assert.equal(log.filter(x=>x==='ready').length,1);assert.ok(log.includes('interstitial'));assert.equal(log.filter(x=>x==='rewarded').length,2);
 ...
 console.log('Yandex build: real UI, opt-in, single reward, cancellation, transition and independent pause/resume passed.');
}catch(error){
 fs.mkdirSync('smoke-artifacts',{recursive:true});await page.screenshot({path:'smoke-artifacts/yandex-failure.png'}).catch(()=>{});
 const state=await page.evaluate(()=>({state:window.__NESI_DEMO_GAME__?.state,blocked:window.__NESI_DEMO_GAME__?.externalBlocked,adLog:window.__adLog,hints:window.__NESI_PREFS__?.value.hints,menu:document.querySelector('#pause-screen')?.outerHTML,trace:window.__uiTrace})).catch(()=>null);
 fs.writeFileSync('smoke-artifacts/yandex-failure.json',JSON.stringify({error:String(error),errors,state},null,2));throw error;
